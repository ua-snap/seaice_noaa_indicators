...
def convert_GTiff( fn, output_path='/workspace/Shared/Tech_Projects/SeaIce_NOAA_Indicators/project_data/nsidc_0051/GTiff/north' ):
	'''
...
if __name__ == '__main__':
	...
	os.chdir('/workspace/Shared/Tech_Projects/SeaIce_NOAA_Indicators/project_data/nsidc_0051')
	
	# DOWNLOAD ALL DAILY DATA NSIDC 0051
	username = "LearningToFly"
	password = "<pwd>" # insert password here...
	...
	path = '/workspace/Shared/Tech_Projects/SeaIce_NOAA_Indicators/project_data/nsidc_0051/raw/daily'
	os.chdir( path )

	filenames = [ os.path.join(r,fn) for r,s,files in os.walk( path ) for fn in files if fn.endswith( '.bin' )]

	pool = mp.Pool( 32 )
	out = pool.map( convert_GTiff, filenames )
	pool.close()
	pool.join()
