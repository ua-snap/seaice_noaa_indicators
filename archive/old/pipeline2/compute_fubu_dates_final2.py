...
def freezeup_start( ds_sic, summer_mean, summer_std, year ):
    ...
    mask = (daily_vals.isel( time=0 ).data == -9999) & (np.isnan(daily_vals.isel( time=0 ).data))

    # make the oob values na temporarily
    daily_vals.data[np.where(daily_vals.data == -9999)] = np.nan 
    ...
    ordinal_days_freezeup_start[ mask ] = -9999

    # if the Aug-Sept mean/threshold is >25%, then that date is NAN. [from Mark/Hajo]
    ordinal_days_freezeup_start[smean > .25] = -1
...
def freezeup_end( ds_sic, winter_mean, freezeup_start_arr, year ):
    ...
    mask = daily_vals.isel( time=0 ).data == -9999
    ...
    # mask it
    ordinal_days_freezeup_end[ mask ] = -9999
...
def breakup_start( ds_sic, winter_mean, winter_std, summer_mean, year ):
    ...
    ordinal_days_breakup_start[ mask ] = -9999 # mask it
    ...
    ordinal_days_breakup_start[ np.where(summer > .40) ] = -1

    # if the day chosen is the last day of the time window (or beyond), make it NA <-- new from Hajo
    ordinal_days_breakup_start[ ordinal_days_breakup_start + 1 == end_ordinalday ] = -1
...
def breakup_end( ds_sic, summer_mean, summer_std, year ):
    ...
    ordinal_days_breakup_end[ summer.values > .25 ] = -1

    # if it is the last day of the time-window, (sept.30th) make it NA
    ordinal_days_breakup_end[ ordinal_days_breakup_end >= end_ordinalday ] = -1
    
    # mask it 
    ordinal_days_breakup_end[ mask ] = -9999
...
if __name__ == '__main__':
    ...
    # fn = '/atlas_scratch/malindgren/nsidc_0051/smoothed/NetCDF/nsidc_0051_sic_nasateam_1978-2017_north_smoothed.nc'
    ...
    # make a no data mask
    mask = ds_sic.isel(time=0).data == -9999 # landpole
    ...
    # DUMP OUT THE FUBU ALL YEARS VALUES AS GTIFF
    for metric in metrics:
        arr = ds_fubu[metric].values 
        output_filename = os.path.join(base_path,'outputs','GTiff','{}_{}-{}{}_ordinal_north_smoothed.tif'.format(metric, begin, end, suffix))
        meta.update(count=39)
        with rasterio.open( output_filename, 'w', **meta ) as out:
            out.write( arr )
